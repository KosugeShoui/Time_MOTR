...
class MOTR(nn.Module):
    ...
    def _forward_single_image(self, samples, time_frame , track_instances: Instances):
        #time frame --> tensor
        #print(samples.shape)
        #print('sample = ',type(samples))
        #Nested Tensor
    
        #tensor_list = samples.unbind()  # .unbind() を使ってリストに変換
        # リスト内のテンソルを同じ長さにパディング


        
        features, pos = self.backbone(samples)
        #print('feature length = ',len(features)) --> 3
        #print(features.shape) --> list
        #print('feature length = ',features)
        
        #マルチスケール3つ分の特徴量が出てくる
        #next_features, next_pos = self.backbone(next_samples)
        #print('next feature length = ',len(next_features)) --> 3
        ...
        for l, feat in enumerate(features):
            ...
            #src1 = src.cpu().detach().numpy()
            #print(src1.shape) 
            # #-->(1,2048,29,39)
            # (1, 512, 116, 155)(1, 1024, 58, 78)(1, 2048, 29, 39)  ???
            #print(src1.shape)
            #print(len(src1[0,:,0,0]))
            #feature map save 
            """
            if len(src1[0,:,0,0]) == 2048:
                feat1 = src1[0,:,0,0]
                feat1 = feat1.reshape(32,64)
                #print(feat1.shape)
                plt.imshow(feat1)
                plt.savefig('feature_vis_visem.png')
            """
                
            #plt.imshow(src1)
            #plt.savefig('feature_vis.png')
        ...
        #[1,256,116,155]
        #for num,i in enumerate(srcs):
            #print('frame feature = ',num,i.shape)
            #マルチスケールの特徴マップ分のデータ
            #0 torch.Size([1, 256, 116, 155])
    ...
    def forward(self, data: dict):
        ...
        for frame_index, frame in enumerate(frames):
            ...
            if frame_index == 0:
                ...
                frame_tensor_time1 = F.interpolate(frame.unsqueeze(0), size=(224, 224), mode='bilinear', align_corners=False).squeeze(0)
                frame_tensor_time2 = F.interpolate(frames[0].unsqueeze(0), size=(224, 224), mode='bilinear', align_corners=False).squeeze(0)
            
            """
            tmp1 = normalize_tensor(frame_tensor_time1.cpu().detach().numpy().copy())
            tmp2 = normalize_tensor(frame_tensor_time2.cpu().detach().numpy().copy())
            fig, axes = plt.subplots(1, 2, figsize=(10, 5))
            axes[0].imshow(tmp1.transpose(1,2,0), cmap='gray')
            axes[0].set_title('frame 1')
            axes[1].imshow(tmp2.transpose(1,2,0), cmap='gray')
            axes[1].set_title('frame 2')
            plt.tight_layout()
            plt.show()
            fig.savefig('input_time_scale_image_{}.png'.format(count))
            """
            ...
            if self.use_checkpoint and frame_index < len(frames) - 2:
                def fn(frame, *args):
                    # Tensor to Nested Tensor
                    
                    #1frame分の推論を実行 #frame --> 1frame分しかない
                    frame = nested_tensor_from_tensor_list([frame])
                    
                    #2フレーム目の入力
                    #print('check')
                    #next_frame = nested_tensor_from_tensor_list([frame])
                    
                    tmp = Instances((1, 1), **dict(zip(keys, args)))
                    #推論結果、bboxの座標やクラス確率など
                    frame_res = self._forward_single_image(frame,cat_frames_time,tmp)
                    return (
                ...
                frame = nested_tensor_from_tensor_list([frame])
                #2フレーム目の入力
                #next_frame = nested_tensor_from_tensor_list(frames[frame_index + 1])
                frame_res = self._forward_single_image(frame, cat_frames_time, track_instances)
